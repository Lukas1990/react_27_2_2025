...
function GetWheater(props) {

  const {onWeatherSubmit, phrase} = props
  ...
  function getData() {
    ...
    const url = "https://api.open-meteo.com/v1/forecast?latitude="+latitude+"&longitude="+longitude+"&daily=weather_code,temperature_2m_max,rain_sum,snowfall_sum,wind_speed_10m_max&timezone=auto&forecast_days=3"
    ...
    axios.get(url).then(response => {
      ...
      for (let i = 0; i < old_data.time.length; i++) {
        let date = new Date(old_data.time[i])
